...
CSSplitter::CSSplitter()
{
	...
	m_nSplitterWidth			= 5;   // the width of slitter
	...
	m_rcWndLeft.SetRect(0, 0, 0, 0);
	m_rcWndRight.SetRect(0, 0, 0, 0);
	m_rcWndTop.SetRect(0, 0, 0, 0);
	m_rcWndBottom.SetRect(0, 0, 0, 0);;
	m_rcVertSplitter.SetRect(0, 0, 0, 0);
	m_rcVertBar.SetRect(0, 0, 0, 0);
	m_rcHorizSplitter.SetRect(0, 0, 0, 0);
	m_rcHorizBar.SetRect(0, 0, 0, 0);
}
		...
		UINT	nMaxRight)
{
	...
	int nLeftPaneWidth = pApp->GetProfileInt(LPCTSTR(strKey), _T("VertSplitPos"), 2*nMaxLeft);
		...
		UINT	nMaxBottom)
{
	...
	m_nBottomPaneHeight = pApp->GetProfileInt(LPCTSTR(strKey), _T("HorizSplitPos"),  2*nMaxBottom );
...
void CSSplitter::OnDrawVertBar(CDC &dc, CRect &rc)
{
	...
	dc.PatBlt(
		rc.left + m_rcVertSplitter.left + m_nSplitterWidth-2, 
		rc.top, 
		m_nSplitterWidth, 
		rc.Height(), 
		PATINVERT);
...
void CSSplitter::OnDestroy() 
{
	...
	{
		pApp->WriteProfileInt(LPCTSTR(strKey), _T("VertSplitPos"), m_rcWndLeft.right );	
	}
	...
	{
		strKey.Format(_T("%s%d"), _T("splt\\init_h"), m_nID);
		pApp->WriteProfileInt(LPCTSTR(strKey), _T("HorizSplitPos"),  m_rcWndBottom.Height());
	}
...
void CSSplitter::OnPaint()
{
	...
	m_rcWndLeft, m_rcWndRight;
	m_rcWndTop, m_rcWndBottom;
	...
	GetClientRect(rc);
	//dc.FillSolidRect(rc, RGB(137,140,149));
